...
trait WorkerConnector {
  ...
  def askConnection(): Future[ActorRef]
...
object WorkerConnector extends Logger {
  ...
  sealed trait Event
  object Event {
    final case class AskConnection(resolve: Promise[ActorRef]) extends Event
    final case class Connected(worker: ActorRef) extends Event
  ...
  def default(
    ...
    def forShared(id: String, ctx: ContextConfig): Future[WorkerConnector] = {
      ...
      logger.info(s"HERE _ STARTING WORKER $id")
      regHub.waitRef(id, spawnSettings.timeout).flatMap(ref => {
        logger.info("RECEIVED REGISTRATION")
        val ready = Promise[ActorRef]
        af.actorOf(WorkerConnection.props(id, mkWorkerInitInfo(ctx), ref, ready, 1 minute))
        ready.future
      }).map(connection => {
        //TODO shutdown
        new WorkerConnector {
          override def askConnection(): Future[ActorRef] = Future.successful(connection)
    ...
    def forExclusive(id: String, ctx: ContextConfig): Future[WorkerConnector] = {
      ...
      val connector = new WorkerConnector {
        ...
        override def askConnection(): Future[ActorRef] = {
