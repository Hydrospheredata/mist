...
object RunnerCommand2 {
  ...
  def mkSparkSubmitCmd(
    name: String,
    initInfo: WorkerInitInfo,
    mistWorkerJar: String,
...
//    def workerLocalPath: String = Paths.get(mistHome, "mist-worker.jar").toRealPath().toString
  ...
  class Docker(
    ...
    port: Int,
    masterHost: String
  ) extends RunnerCommand2 with Logger {
    ...
    override def onStart(name: String, initInfo: WorkerInitInfo): WorkerProcess = {
      import scala.collection.JavaConverters._
      val entryPoint = mkSparkSubmitCmd(
        name, initInfo, "/usr/share/mist/mist-worker.jar", "/usr/share/spark/bin/spark-submit", masterHost)
      //val entryPoint = List("/usr/share/mist/bin/mist-worker", "--master", s"$masterHost:2551")
      val resp = client.createContainerCmd(image).withEntrypoint(entryPoint.asJava).exec()
      val resp2 = client.startContainerCmd(resp.getId)

      logger.info(s"Docker response $resp, $resp2")
      NonLocal
    }
  }
  ...
  def create(masterAddress: String, workersSettings: WorkersSettingsConfig): RunnerCommand2 = {
    ...
    runnerType match {
      ...
      case "docker" =>
        import workersSettings.dockerConfig._
        new Docker(image, dockerHost, dockerPort, masterAddress)
