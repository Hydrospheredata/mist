...
class ContextsStorageSpec extends FunSpec with Matchers with BeforeAndAfter {
  ...
  it("should update") {
    ...
    val ctx = ContextConfig("new", Map.empty, Duration.Inf, 50, false, "weq", RunMode.Shared, 10 second, 5, ServerDefault)
  ...
  it("should return precreated") {
    val contexts = testStorage()
    val ctx = ContextConfig("new", Map.empty, Duration.Inf, 50, true, "weq", RunMode.Shared, 10 second, 5, ServerDefault)
  ...
  it("should override settings") {
    ...
    val ctx = ContextConfig("foo", Map.empty, Duration.Inf, 50, true, "FOOOOPT", RunMode.Shared, 10 second, 5, ServerDefault)
