...
class PythonRunner(jobConfiguration: JobConfiguration, jobFile: JobFile, contextWrapper: ContextWrapper) extends Runner {
  ...
  val configurationWrapper = new ConfigurationWrapper(JobConfiguration(jobFile.file.getPath, jobConfiguration.className, jobConfiguration.name, jobConfiguration.parameters, jobConfiguration.external_id))
