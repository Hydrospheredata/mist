...
trait SparkSpecificEncoder {
  ...
  implicit val datasetEncoder = new Encoder[Dataset[_]] {
      ...
      val data = ds.queryExecution.toRdd.mapPartitions(iter => {
        new Iterator[JsLikeData] {
          ...
          override def next(): JsLikeData = {
            val row = iter.next()
            convertRow(row, schema, fieldsConverters)
          }
        }
      })
    ...
    def convertRow(row: InternalRow, st: StructType): JsLikeData = {
...
trait DefaultEncoders extends PrimitiveEncoders with CollectionsEncoder with SparkSpecificEncoder
